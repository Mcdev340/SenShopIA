...
export interface UseLoggerReturn {
  ...
  /** Log d'exception */
  exception: (error: any, context?: Record<string, any>, tags?: string[]) => void;
  /** Log de performance */
...
export const useLogger = (
  ...
  const createChild = useCallback((childTags: string[]): Logger => {
  ...
  const debug = useCallback((message: string, data?: any, extraTags?: string[]) => {
    log(LogLevel.DEBUG, message, data, undefined, extraTags);
  }, [log]);

  const info = useCallback((message: string, data?: any, extraTags?: string[]) => {
    log(LogLevel.INFO, message, data, undefined, extraTags);
  }, [log]);

  const warn = useCallback((message: string, data?: any, extraTags?: string[]) => {
    log(LogLevel.WARN, message, data, undefined, extraTags);
  }, [log]);

  const error = useCallback((message: string, error?: any, extraTags?: string[]) => {
  ...
  const exception = useCallback((
    ...
    log(LogLevel.ERROR, error instanceof Error ? error.message : String(error), context, error, extraTags);
  }, [log]);

  // ============ MÉTHODES SPÉCIALISÉES ============

  const performance = useCallback((label: string, duration: number, data?: any) => {
  ...
  const component = useCallback((action: string, data?: any) => {
  ...
  const event = useCallback((name: string, data?: any) => {
    info(`Event: ${name}`, {
      name,
      ...data,
      _type: 'event',
  ...
  const group = useCallback((name: string, fn: () => void) => {
    if (!enabledRef.current) return;
    const loggerInstance = createChild([]);
    loggerInstance.group(name, fn);
  }, [createChild]);

  const groupCollapsed = useCallback((name: string, fn: () => void) => {
    if (!enabledRef.current) return;
    const loggerInstance = createChild([]);
  ...
  const child = useCallback((childTags: string[]): UseLoggerReturn => {
    const childContext = context ? `${context}:${childTags.join(':')}` : childTags.join(':');
    const childOptions: UseLoggerOptions = {
      ...options,
      tags: [...fullTags, ...childTags],
      component: component || childTags[0],
    };
...
export const useComponentLogger = (
  ...
  const stateChange = useCallback((stateName: string, newValue: any) => {
...
export const useApiLogger = (
  ...
  /** Logger une réponse */
  response: (method: string, endpoint: string, response: any, duration?: number) => void;
  /** Logger une erreur API */
  error: (method: string, endpoint: string, error: any, duration?: number) => void;
  /** Logger une requête avec durée */
  timing: (method: string, endpoint: string, duration: number, success: boolean) => void;
  /** Logger une requête complète avec durée */
  logRequest: <T>(method: string, endpoint: string, fn: () => Promise<T>) => Promise<T>;
} => {
  ...
  const fullUrl = useCallback((endpoint: string) => {
    return `${baseUrl}${endpoint}`;
  }, [baseUrl]);

  const request = useCallback((method: string, endpoint: string, data?: any) => {
...
/**
 * Type du retour du hook useLogger
 */
