...
interface Activity {
  id: string;
  type: 'order' | 'user' | 'product' | 'payment' | 'delivery' | 'message' | 'system';
  title: string;
  description: string;
  timestamp: Date;
  user?: {
...
const activityIcons = {
  order: { icon: ShoppingBag, color: 'text-blue-500 bg-blue-100 dark:bg-blue-900/20' },
  user: { icon: User, color: 'text-green-500 bg-green-100 dark:bg-green-900/20' },
  product: { icon: Package, color: 'text-purple-500 bg-purple-100 dark:bg-purple-900/20' },
  payment: { icon: CreditCard, color: 'text-orange-500 bg-orange-100 dark:bg-orange-900/20' },
  delivery: { icon: Truck, color: 'text-indigo-500 bg-indigo-100 dark:bg-indigo-900/20' },
  message: { icon: MessageCircle, color: 'text-pink-500 bg-pink-100 dark:bg-pink-900/20' },
  system: { icon: AlertCircle, color: 'text-gray-500 bg-gray-100 dark:bg-gray-800' },
};
        ...
        <div className="space-y-4">
          {loading ? (
            ...
            displayActivities.map((activity) => {
              const Icon = activityIcons[activity.type]?.icon || activityIcons.system.icon;
              const iconColor = activityIcons[activity.type]?.color || activityIcons.system.color;
